...
export const reqTrademarkSelect = (page: number, limit: number) =>
  request.get<any, TrademarkResponseData>(
    API.TRADEMARK_SELECT + `/${page}/${limit}`,
  )
export const reqTrademarkSave = (data: any) =>
  request.post(API.TRADEMARK_SAVE, data)
export const reqTrademarkDelete = (id: number) =>
  request.delete(API.TRADEMARK_DELETE + `/${id}`)
export const reqTrademarkUpdate = (data: any) =>
  request.put(API.TRADEMARK_UPDATE, data)
